...
public class RunUtils {
    ...
    private void deal(String run){
        if (run.startsWith("[")){
            ...
            String dealStr = MsgUtils.color(run.replace("["+tag+"]","").replace("%player%",p.getName()));
...
//                        );
                ...
                case "broad_guild":
                    if ((new Random()).nextInt(101) <= chance*100){
                        ...
                        if (user.hasGuild()){
                            ...
                            MsgUtils.sendGuildMessage(guild.getMembers(),dealStr);
                        }
                    }
                    break;
                case "guild_money":
                ...
                case "guild_activity":
                    if ((new Random()).nextInt(101) <= chance*100) {
                        User user = legendaryGuild.getUserManager().getUser(p.getName());
                        if (user.hasGuild()) {
                            GuildAPI.addGuildActivity(p,null,Double.parseDouble(dealStr), true);
                        }
                    }
                    break;
                case "guild_exp" :
                    ...
                    if (user.hasGuild()) {
                        Guild guild = legendaryGuild.getGuildsManager().getGuild(user.getGuild());
                        if (guild != null) {
                            GuildAPI.addGuildExp(p.getName(), guild, Double.parseDouble(dealStr));
                        }
                    }
            }
            return;
        }
        legendaryGuild.info("配置格式出错:必须以[标识]开头。比如[console]xxxx -> "+run, Level.SEVERE);
    }
